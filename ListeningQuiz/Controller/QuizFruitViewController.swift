...
class QuizFruitViewController: UIViewController {
    ...
    func loadQuiz(quizNumber:Int){
        ...
        let quizItem = quizFruitArray[quizNumber]
        ...
        //進度條設定
        currentQuizNumber += 1
        progressBar.progress = Float(currentQuizNumber)/Float(quizFruitArray.count)
    }
    // 滑動slider時顯示數值
